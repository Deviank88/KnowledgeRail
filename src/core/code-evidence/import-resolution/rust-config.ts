...
export interface CargoConfig { name?: string; roots: string[]; members: string[]; notices: string[] }
export const CARGO_MANIFEST: ProjectManifestSpec = { fileName: "Cargo.toml", parse(content) {
  ...
  try {
    ...
    for (const member of manifestStrings(workspace.members) ?? []) {
      ...
      else if (/[*?{}\[\]]/u.test(member)) notices.add("unsupported_cargo_workspace_glob");
  ...
  return { ...(typeof name === "string" ? { name: name.replace(/-/gu, "_") } : {}), roots, members, notices: [...notices] } satisfies CargoConfig;
}, notices(value) {
  return (value as CargoConfig).notices;
}, references(value, path) {
  return (value as CargoConfig).members.map((member) => {
