...
It hand-labels 144 symbols in 32 real-world-shaped source files totaling 1,082 source lines across Java, Apex, C#, Go, Rust, PHP, C, C++, and Python. The mixed-repository benchmark adds two LWC files, bringing that tree to 34 files and 1,099 lines. Baseline v3 separately pins both sets of file, line, and byte counts, the label count, the corpus digest, and a generous upper bound for isolated Python-corpus extraction, then requires at least 0.95 precision and 0.90 recall per language. Optional labels also pin definitions, routes, imports, calls, configuration keys, and database references instead of measuring only symbol inventory. Adversarial masks cover text blocks, C# interpolations containing nested quoted expressions, raw/verbatim strings, Rust nested comments and lifetimes, PHP HTML interleaving and heredoc, C/C++ macro/raw-string cases, and Python prefixes, nested-quote f-strings, triple strings, Unicode byte width, tab stops, multiline continuations, exact test-name boundaries, Flask Blueprint routes, and one-line docstrings. A bounded test additionally scans 5,000 nested delimiters and a 512 KB triple string. C/C++ fixtures pin access-label start lines, pointer-return functions, namespace-qualified free functions, and constructors. The mixed pass also pins file reuse after an unchanged rebuild.
